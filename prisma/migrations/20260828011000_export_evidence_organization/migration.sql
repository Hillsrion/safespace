...
CREATE OR REPLACE FUNCTION safespace_private.export_own_contributions()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
  ...
  RETURN jsonb_build_object(
      ...
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'media', COALESCE((
          SELECT jsonb_agg(jsonb_build_object(
            ...
            'evidenceCategory', media."evidenceCategory", 'caption', media.caption, 'sortOrder', media."sortOrder"
          ) ORDER BY media."sortOrder", media.id)
    ...
    'uploadedMedia', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'createdAt', media."createdAt", 'evidenceCategory', media."evidenceCategory",
        'caption', media.caption, 'sortOrder', media."sortOrder"
      ) ORDER BY media."sortOrder", media.id)
