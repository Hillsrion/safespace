...
| Exploitation réelle | La CI vérifie désormais qu’un dump PostgreSQL se restaure dans une base jetable avec migrations, tables, RLS, politiques et fonctions privées intactes. Il reste à vérifier TLS, chiffrement et droits des fournisseurs, restauration d’une sauvegarde réelle avec RPO/RTO mesurés, observabilité, charge et traitement effectif des suppressions sur le déploiement cible. Les tests locaux et les images CI ne prouvent pas cette configuration. |
