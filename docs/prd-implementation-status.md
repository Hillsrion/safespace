...
les requêtes de recherche applicatives avec `EXPLAIN ANALYZE` et refuse une
régression vers un balayage séquentiel ou au-delà d'un budget CI de deux secondes.
...
| Recherche et performance | Les résultats sont filtrés par les droits. Les requêtes exactes de publications, noms et identifiants partiels sont maintenant mesurées sur PostgreSQL réel avec 20 000 lignes par type ; la CI impose l’usage des index FTS/trigrammes, l’absence de balayage séquentiel des tables recherchées et un budget de deux secondes. Il reste à fixer un SLO produit, mesurer la concurrence de bout en bout et valider latence/charge sur l’infrastructure de production ; ce garde-fou de plan n’est pas un test de capacité. |
| Exploitation réelle | La CI vérifie désormais qu’un dump PostgreSQL se restaure dans une base jetable avec migrations, tables, index, extensions, RLS, politiques et fonctions privées intactes, y compris entre un client récent et un serveur plus ancien. Il reste à vérifier TLS, chiffrement et droits des fournisseurs, restauration d’une sauvegarde réelle avec RPO/RTO mesurés, observabilité, charge et traitement effectif des suppressions sur le déploiement cible. Les tests locaux et les images CI ne prouvent pas cette configuration. |
