...
describe("built worker lifecycle logic", () => {
  it("exits unsuccessfully on failed deletion, cleans handlers, and never logs object details", async () => {
    ...
    mocks.query.mockResolvedValueOnce([role]).mockResolvedValueOnce([job]).mockResolvedValueOnce([{ fail_media_deletion_job: true }]);
  ...
  it("does not report success when SQL lease completion is fenced out", async () => {
    mocks.query.mockResolvedValueOnce([role]).mockResolvedValueOnce([job]).mockResolvedValueOnce([{ complete_media_deletion_job: false }]);
  ...
  it("polls again after the cadence and stops promptly without another claim", async () => {
    ...
    mocks.query.mockResolvedValueOnce([role]).mockResolvedValue([]);
