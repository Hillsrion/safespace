...
import { deletePost, getPostWithSpace } from "~/db/repositories/posts/actions.server";
...
export async function action({ request, params }: ActionFunctionArgs): Promise<ActionResult<'deleted'>> {
  const { postId } = params;
  if (!postId) {
