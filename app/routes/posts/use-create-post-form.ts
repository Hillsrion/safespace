...
import { PostSeverity } from '@/generated/prisma/client'; // Path to Prisma enum
...
export function useCreatePostForm({ submittedValues, mockPeople = [] }: UseCreatePostFormProps) {
  const [name, setName] = useState(submittedValues?.name || '');
  const [instagramHandle, setInstagramHandle] = useState(submittedValues?.instagramHandle || '');
  const [description, setDescription] = useState(submittedValues?.description || '');
  
  // Handle severity potentially being a string from submittedValues
  const initialSeverity = typeof submittedValues?.severity === 'string' 
    ? (submittedValues.severity as PostSeverity) // Cast if string matches enum values
    : submittedValues?.severity || PostSeverity.medium;
  const [severity, setSeverity] = useState<PostSeverity>(initialSeverity as PostSeverity);

  // Handle booleans potentially being 'on' string from submittedValues
  const initialPostAnonymously = typeof submittedValues?.postAnonymously === 'string'
    ? submittedValues.postAnonymously === 'on'
    : !!submittedValues?.postAnonymously;
  const [postAnonymously, setPostAnonymously] = useState<boolean>(initialPostAnonymously);

  const initialAdminOnly = typeof submittedValues?.adminOnly === 'string'
  ...
  const [selectedNamePerson, setSelectedNamePerson] = useState<Person | null>(null);
  const [selectedInstagramPerson, setSelectedInstagramPerson] = useState<Person | null>(null);
  ...
  }, [selectedNamePerson, mockPeople]); // mockPeople was in dep array, but not directly used here. Removed.

  const handleInstagramChange = useCallback((value: string) => {
    setInstagramHandle(value);
    if (selectedInstagramPerson && value !== selectedInstagramPerson.instagram) {
        setSelectedInstagramPerson(null);
    }
  }, [selectedInstagramPerson, mockPeople]); // mockPeople was in dep array, but not directly used here. Removed.


  const handleNameSelect = useCallback((person: Person | null) => {
    ...
    if (person) {
      ...
      if (!instagramHandle || instagramHandle !== person.instagram) { 
        ...
        const matchingInstagramPerson = mockPeople.find(p => p.instagram === person.instagram && p.id === person.id);
  ...
  }, [instagramHandle, mockPeople, setInstagramHandle, setSelectedInstagramPerson]); // Added setInstagramHandle and setSelectedInstagramPerson

  const handleInstagramSelect = useCallback((person: Person | null) => {
    setSelectedInstagramPerson(person);
    if (person) {
      setInstagramHandle(person.instagram);
      if (!name || name !== person.name) { 
        setName(person.name);
        const matchingNamePerson = mockPeople.find(p => p.name === person.name && p.id === person.id);
        setSelectedNamePerson(matchingNamePerson || null);
      }
    }
  }, [name, mockPeople, setName, setSelectedNamePerson]); // Added setName and setSelectedNamePerson

  const resetForm = useCallback((imageUploadRef?: React.RefObject<ImageUploadRef>) => {
    setName('');
    setInstagramHandle('');
    setDescription('');
  ...
  useEffect(() => {
    if (submittedValues) {
        ...
        const newSeverity = typeof submittedValues.severity === 'string'
        ...
        const newPostAnonymously = typeof submittedValues.postAnonymously === 'string'
        ? submittedValues.postAnonymously === 'on'
        : !!submittedValues.postAnonymously;
        setPostAnonymously(newPostAnonymously);

        const newAdminOnly = typeof submittedValues.adminOnly === 'string'
        ? submittedValues.adminOnly === 'on'
        : !!submittedValues.adminOnly;
        ...
        const currentNameMatch = mockPeople.find(p => p.name === (submittedValues.name || ''));
        setSelectedNamePerson(currentNameMatch || null);
        
        const currentIgMatch = mockPeople.find(p => p.instagram === (submittedValues.instagramHandle || ''));
