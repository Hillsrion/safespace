...
import { parseFormData, type File as MJacksonFile } from '@mjackson/form-data-parser';
import { R2FileStorage } from '@edgefirst-dev/r2-file-storage';

import { getSession, commitSession } from '~/services/session.server.ts';
import { getCurrentUser } from '~/services/auth.server.ts';
import { createPost } from '~/db/repositories/posts/queries.server.ts';
...
const createPostSchema = z.object({
  ...
  postAnonymously: z.preprocess(value => value === 'on' || value === true, z.boolean()),
  adminOnly: z.preprocess(value => value === 'on' || value === true, z.boolean()),
...
export const action = async ({ request, context }: ActionFunctionArgs) => { 
  ...
  const r2Binding = context.cloudflare.env[R2_BINDING_NAME] as R2Bucket | undefined;

  if (!r2Binding) {
    console.error("R2_BUCKET binding not found in Cloudflare context.");
    return json({ errors: { form: "Configuration de stockage de fichiers manquante côté serveur." } }, { status: 500 });
  }
  const storage = new R2FileStorage(r2Binding);

  const handleFileUpload = async (fileInfo: MJacksonFile): Promise<UploadedEvidenceDetails | null> => {
    if (fileInfo.name !== 'evidenceFile' || !fileInfo.filename) {
      return null; 
    }
    
    if (!ALLOWED_IMAGE_TYPES.includes(fileInfo.contentType)) {
      throw new Error(`Type de fichier non supporté: ${fileInfo.contentType}. Types acceptés: JPG, PNG, GIF, WEBP.`);
    }
    ...
    for await (const chunk of fileInfo.data) { 
      ...
      if (currentSize > MAX_FILE_SIZE_MB * 1024 * 1024) {
        throw new Error(`Fichier trop volumineux. Taille max: ${MAX_FILE_SIZE_MB}MB.`);
      }
      chunks.push(chunk);
    }
    const buffer = Buffer.concat(chunks);
    const file = new File([buffer], fileInfo.filename, { type: fileInfo.contentType });
    ...
    try {
      ...
      console.error("R2 Upload Error with @edgefirst-dev/r2-file-storage:", uploadError);
      throw new Error(`Échec du téléversement du fichier vers R2: ${uploadError.message}`);
    }
  };
  ...
  try {
    ...
    if (evidenceFilesMetaData.length > 0) {
        console.warn(`Error during form processing after ${evidenceFilesMetaData.length} file(s) uploaded to R2. Orphaned files:`, evidenceFilesMetaData.map(f => f.key));
    }
    return json({ errors: { form: error.message || "Erreur lors du traitement du formulaire ou du téléversement." } }, { status: 400 });
  }
  ...
  if (!validationResult.success) {
    if (evidenceFilesMetaData.length > 0) {
      console.warn(
        ...
        `Orphaned files: ${evidenceFilesMetaData.map(f => f.key).join(', ')}. Errors:`, 
  ...
  if (spaceId === "hardcoded-space-id-for-testing") { 
    console.warn("Using hardcoded spaceId for post creation. Replace with actual space selection logic.");
  }
  if (!actualUserId && !validatedData.postAnonymously) {
      return json({ errors: { form: "Vous devez être connecté pour créer un post non anonyme." } }, { status: 403 });
  }
  ...
  try {
    ...
    session.flash("toastMessage", `Post "${newPost.id.substring(0,8)}..." créé avec succès (multi-file R2) !`);
    ...
    if (validatedData.evidenceFiles && validatedData.evidenceFiles.length > 0) {
      const uploadedKeys = validatedData.evidenceFiles.map(f => f.key).join(', ');
      console.warn(`DB error after uploading files to R2. Orphaned files: ${uploadedKeys}. Consider implementing deletion.`);
    }
    return json({ 
        errors: { form: `La création du post a échoué: ${dbError.message || "Erreur de base de données."}` }, 
        submittedValues: objectToValidate 
    }, { status: 500 });
  }
};
