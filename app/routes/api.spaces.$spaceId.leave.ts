...
import { leaveSpaceSchema } from "~/lib/schemas/spaceSchemas";
  ...
  if (!validationResult.success) {
    return json({ error: "Validation failed", issues: validationResult.error.flatten() }, { status: 400 });
  }
  ...
  try {
    ...
    if (result.count === 0) {
      ...
      console.log(`User ${user.id} was not a member of space ${spaceId}, or already left.`);
    }

    return json({ success: true, message: `Successfully left space ${spaceId}` });
  } catch (error) {
    console.error(`Error processing leave space request for user ${user.id}, space ${spaceId}:`, error);
    return json({ error: "Failed to leave space" }, { status: 500 });
  }
};
